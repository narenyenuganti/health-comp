...
| Integrated-candidate Simulator launch | Current-main candidate `0e08e2a` was built as the exact staging Simulator bundle with frozen package resolution; its embedded simulated capability and public configuration boundaries were verified, and a state-preserving over-install retained one authenticated profile root with seven files. The installed executable matched SHA-256 `4308ed164cabe956eaa785038700c16968715cf1516e6e86be5492c622865df7`. Following explicit approval, one launch action ran at `2026-08-23T18:11:40Z` with no retry. Accessibility exposed only a protected/blank surface; HealthComp was already stopped at settled readback, and no attributable app-owned transport, bundle-lifecycle, or diagnostic lines were available. One profile root, seven files, one primary journal, and two server-cursor-directory files remained; no profile file changed after the launch timestamp. The privacy-safe receipt SHA-256 is `cc8c28a12bbf1b2b5773c3e479972874de61cb9b74e67b3e13466faef035d6e4`. This proves exact-candidate build/install and authenticated local preservation, not successful runtime execution of the Activity-isolation fix | PARTIAL |
