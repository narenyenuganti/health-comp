...
| Authenticated refresh | The prior `9d19937` relaunch reproduced `Some competition activity could not be refreshed.` and privacy-safe hosted readbacks isolated two terminal descriptors. PR #34 corrected the terminal-descriptor boundary, and then-selected artifact `dd0ed68` reached authenticated Sharing without a warning. Exact-current `6ee1d22` reached authenticated Sharing on the physical phone and sequentially on both Simulator accounts, but every retained UI readback still showed the historical-activity refresh warning. Account B mounted one four-envelope journal after zero-root teardown; the later Account A recovery again started from zero roots and mounted one distinct five-envelope journal. Both bounded `6ee1d22` Simulator windows returned only HTTP 200/204 markers with no non-2xx, connection-loss, `-7026`, or missing-entitlement marker. The later exact-candidate `0e08e2a` recovery retry again reached authenticated Sharing with the same generic warning and `No Competition`; its process window instead contained 72 CFNetwork `-1005` markers despite a successful credential-free staging TLS probe. A fresh exact-candidate attempt then consumed exactly one Account A pull-to-refresh after credential-free TLS again passed. The warning and `No Competition` remained through a bounded observation, while the complete profile tree stayed byte-for-byte unchanged with one five-envelope journal, zero inventory entries, zero outbox entries, and zero App Attest files. Its safe log aggregate contained no connection-loss, Apple-auth, missing-entitlement, or crash marker but no precise attributable HTTP status. The Activity-isolation behavior still lacked a remote competition to materialize, so warning-free exact-current refresh remains unproven | PARTIAL |
...
1. Run a privacy-safe read-only hosted inventory for Account A to determine
   whether staging currently has any eligible competition that the exact
   candidate should materialize. Retain only aggregate lifecycle and membership
   counts, not identities or private identifiers. If no eligible competition
   exists, create no replacement without separate approval; if one exists,
   diagnose the authenticated fetch boundary before another device action.
...
- Current-main candidate `0e08e2a` is integrated and green. Its recovered
  Account A session now has one separately approved pull-to-refresh attempt,
  but authenticated Sharing retained `No Competition` plus the historical-
  activity warning. The full single-root, seven-file profile tree and five-
  envelope history remained byte-for-byte unchanged, with zero inventory and
  outbox entries and zero App Attest files. This does not select the candidate
  or prove the Activity-isolation runtime fix; hosted eligibility must be
  established read-only before another device action.
